 ...
 *   `swapDirectionRef` ref still gets a sensible value, but the new
...
export function useJewelrySwap({
    ...
    swapDirectionRef: React.RefObject<number>;
}) {
    ...
    const animateProgress = useCallback(
        (duration: number, onMidpoint: () => void) => {
            ...
            const tick = (now: number) => {
                ...
                transitionProgressRef.current = Math.max(0, p);
    ...
    const swapToIndex = useCallback(
        (target: number, direction: number) => {
            ...
            playSwapTick();
            swapDirectionRef.current = direction;

            animateProgress(700, () => {
                onJewelryChange(target);
            });
        },
        [animateProgress, onJewelryChange, swapDirectionRef]
    );
