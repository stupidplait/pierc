...
// Warm the GLB cache on hover/focus so the first equip doesn't block on a cold
// fetch + parse (the showroom's core interaction). Deduped per id so repeated
// hovers don't re-trigger, and best-effort so a preload failure is harmless.
const preloadedGlb = new Set<string>();
function preloadPieceGlb(j: JewelryWire) {
  if (!j.glbUrl || preloadedGlb.has(j.id)) return;
  preloadedGlb.add(j.id);
  try {
    useGLTF.preload(catalogGlbSrc(j.id, j.glbUrl));
  } catch {
...
export function SlotCard({
  ...
  return (
    <article
      onPointerEnter={() => preloadPieceGlb(j)}
      onFocusCapture={() => preloadPieceGlb(j)}
      className={cn(
      ...
      <div className="relative aspect-square overflow-hidden">
        ...
        {isEquippedHere ? (
          <button
            ...
            onClick={onUnequip}
            title={ct.unequip}
            aria-label={`${j.name} — ${ct.unequip}`}
            className="absolute bottom-1.5 right-1.5 z-20 grid size-8 place-items-center rounded-full bg-accent text-on-primary transition before:absolute before:-inset-1.5 before:content-[''] hover:bg-primary-soft active:scale-95"
          >
            <X className="size-4" />
          </button>
        ) : (
          <button
            ...
            aria-label={`${j.name} — ${canEquip ? ct.equip : ct.listAnchorPrompt}`}
            className="absolute bottom-1.5 right-1.5 z-20 grid size-8 place-items-center rounded-full border border-ink/25 bg-bg/75 text-ink backdrop-blur transition before:absolute before:-inset-1.5 before:content-[''] hover:border-accent hover:bg-accent hover:text-on-primary active:scale-95 disabled:opacity-35 disabled:hover:border-ink/25 disabled:hover:bg-bg/75 disabled:hover:text-ink disabled:active:scale-100"
          >
