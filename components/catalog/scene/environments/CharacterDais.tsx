 ...
 * the look lives in ShowroomStage (it must spin the body + equipped jewelry
 * together), gated on reduced-motion + no selection.
 */
export function CharacterDais({ bg }: { bg: BgVariant }) {
  ...
  return (
    <>
      ...
      {/* Polished reflective dais the figure stands on. NOTE: keep mixStrength /
          blur in sane ranges — an over-amplified reflection (mixStrength was 45)
          overflows the blur passes' half-float precision into NaN, which renders
          as RGB garbage blocks under the model that Bloom then lights up. */}
