...
- Inline `Проверить` link beside the Telegram chat id field at `/admin/settings` → `runTelegramTest` server action → `sendTelegramTest`. Pings the currently-typed chat id with the configured bot and reports the result via toast with a friendly Russian reason ("Telegram-бот не настроен", "Укажите Telegram chat id", "Сообщение отправлено", etc.). The email leg has no test affordance; booking emails still fire on their own.
