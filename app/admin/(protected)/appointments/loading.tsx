// Appointments loading skeleton. The protected group's loading.tsx is
// dashboard-shaped (a Status Board), so without this per-segment override the
// appointments screen would flash the dashboard placeholder on navigation. This
// mirrors app/admin/(protected)/appointments/page.tsx: hero header, the 3-up
// summary strip, the status chip strip + today toggle over the search row, then
// the two-pane console (left rail of grouped rows + right dossier pane).
...
// Varied chip widths so the status strip reads like real labels + counts
// ("Все", "Ожидание", "Подтверждено"…) instead of a row of identical bars.
const CHIP_WIDTHS = ["w-16", "w-28", "w-32", "w-28", "w-24", "w-20"];

// Rail groups with a per-group row count — mirrors the real console, where the
// first day usually carries more rows than later ones.
...
// One left-rail row — tone dot · name · time. Mirrors the console rail item.
function RailRow() {
  return (
    <div className="flex items-center gap-2.5 py-2 pl-3 pr-2.5">
      <div className="size-1.5 shrink-0 rounded-full bg-ink/15" />
      <div className="h-4 w-28 max-w-full flex-1 animate-pulse rounded bg-ink/10" />
      <div className="h-3 w-9 shrink-0 animate-pulse rounded bg-ink/10" />
    </div>
  );
}
...
export default function Loading() {
  return (
    <div aria-hidden="true" className="mx-auto w-full max-w-6xl">
      ...
      {/* Summary strip — total · pending · today. */}
      ...
      {/* Filters — status chip strip + today toggle, then the search row. */}
      <div className="mb-7 flex flex-col gap-3">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="h-9 w-24 animate-pulse rounded-xl bg-card sm:ml-auto" />
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[19rem_1fr]">
        ...
        <div className={`${CARD} h-fit p-6 sm:p-8`}>
          ...
          <div className="mt-2 h-4 w-64 max-w-full animate-pulse rounded bg-ink/10" />
