// Jewelry editor (create/edit) loading skeleton. Without this per-segment
// override the edit route falls back to the catalog list's loading.tsx (hero +
// status strip + list rows), which is the wrong shape. This mirrors
// app/admin/(protected)/jewelry/[id]/edit/page.tsx + components/admin/JewelryForm
// on its default "attributes" tab: top section (title + the Полоса progress
// strip on one line) → tab bar → attributes card (field grid + publication
// block) → command bar (delete left · back + save right).
  ...
  return (
    <div aria-hidden="true" className="mx-auto w-full max-w-6xl pt-2 sm:pt-4">
      <div className="flex flex-col gap-6">
        ...
        {/* Command bar — delete · compact progress · back + save. */}
        <div className="flex items-center gap-3 rounded-2xl border border-line bg-card/95 px-3 py-2.5 sm:px-4 sm:py-3">
          <div className="h-11 w-11 animate-pulse rounded-xl bg-ink/10 sm:w-28" />
